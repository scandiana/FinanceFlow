        ...
        <div class="card" style="padding:1.5rem;">
            <div style="display:flex;justify-content:space-between;">
                <div>
                    ...
                    <p class="text-muted" style="font-size:0.75rem;">{{ $cartao['bandeira'] }} · Final {{ $cartao['final'] }}</p>
                </div>
                <span class="badge badge-success">{{ ucfirst($cartao['status']) }}</span>
            </div>
            <p style="margin-top:1rem;font-size:0.875rem;">Fatura atual</p>
            <p style="font-size:1.25rem;font-weight:600;">{{ \App\Data\MockData::formatMoney($cartao['fatura_atual']) }}</p>
            <p class="text-muted" style="font-size:0.75rem;">Vence em {{ \App\Data\MockData::formatDate($cartao['vencimento']) }}</p>
            <p class="text-muted" style="font-size:0.75rem;margin-top:0.5rem;">Limite: {{ \App\Data\MockData::formatMoney($cartao['limite']) }}</p>
