...
<div class="page-header">
    ...
    <div class="btn-group">
        ...
        <a href="{{ route('fluxo-caixa.create', ['tipo' => 'receita']) }}" class="btn btn-success">
          Nova Receita
        </a>

        <a href="{{ route('fluxo-caixa.create', ['tipo' => 'despesa']) }}" class="btn btn-danger">
            Nova Despesa
        </a>
    </div>
</div>
...
<div class="grid grid-3" style="margin-bottom:1.5rem;">
    <div class="card metric-card"><p class="label">Total de Receitas</p><p class="value text-success">R$ {{ number_format($resumo['receitas'], 2, ',', '.') }}</p></div>
    <div class="card metric-card"><p class="label">Total de Despesas</p><p class="value text-danger">R$ {{ number_format($resumo['despesas'], 2, ',', '.') }}</p></div>
    <div class="card metric-card"><p class="label">Saldo do Período</p><p class="value {{ $resumo['saldo'] >= 0 ? 'text-success' : 'text-danger' }}">R$ {{ number_format($resumo['saldo'], 2, ',', '.') }}</p></div>
</div>
            ...
            <tbody>
                @forelse($movimentacoes as $m)
                    <tr>
                        ...
                        <td><td>{{ $m['categoria']['nome'] ?? '-' }}</td></td>
                        <td><td>{{ $m['conta']['nome'] ?? '-' }}</td></td>
