...
<div class="card metric-card" style="margin-bottom:1.5rem;"><p class="label">Total de receitas</p><p class="value text-success">{{ \App\Data\MockData::formatMoney($total) }}</p><p class="text-muted" style="font-size:0.75rem;margin-top:0.25rem;">{{ $movimentacoes->count() }} lançamento(s)</p></div>
            ...
            <tbody>
                @forelse($movimentacoes as $m)
                    <tr>
                        ...
                        <td>{{ $m['categoria'] }}</td>
                        <td>{{ $m['conta'] }}</td>
                        <td class="text-right text-success">{{ \App\Data\MockData::formatMoney(abs($m['valor'])) }}</td>
                    </tr>
                @empty
