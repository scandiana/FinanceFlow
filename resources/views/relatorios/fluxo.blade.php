...
<div class="grid grid-3" style="margin-bottom:1.5rem;">
    <div class="card metric-card">
        ...
        <p class="value text-success">R$ {{ number_format($resumo['receitas'], 2, ',', '.') }}</p>
    </div>

    <div class="card metric-card">
        <p class="label">Despesas</p>
        <p class="value text-danger">R$ {{ number_format($resumo['despesas'], 2, ',', '.') }}</p>
    </div>

    <div class="card metric-card">
        <p class="label">Saldo</p>
        <p class="value {{ $resumo['saldo'] >= 0 ? 'text-success' : 'text-danger' }}">
            ...
            <tbody>
                @forelse($movimentacoes as $m)
                    <tr>
                        ...
                        <td class="text-right {{ $m['valor'] >= 0 ? 'text-success' : 'text-danger' }}">R$ {{ number_format(abs($m['valor']), 2, ',', '.') }}</td>
                    </tr>
                @empty
