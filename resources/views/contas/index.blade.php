...
<div class="page-header">
    <div>
        ...
        <p>Saldo consolidado: <strong>{{ \App\Data\MockData::formatMoney($saldoTotal) }}</strong></p>
    </div>
...
<div class="grid grid-3" style="margin-bottom:1.5rem;">
    @foreach($contas as $conta)
        <a href="{{ route('contas.show', $conta['id']) }}" class="card link-card metric-card">
            ...
            <p class="value" style="margin-top:0.75rem;">{{ \App\Data\MockData::formatMoney($conta['saldo']) }}</p>
            <p class="text-muted" style="font-size:0.75rem;margin-top:0.5rem;">Ag {{ $conta['agencia'] }} · Cc {{ $conta['numero'] }}</p>
        </a>
    @endforeach
</div>
