            ...
            <tbody>
                @foreach($clientes as $c)
                    <tr>
                        ...
                        <td>{{ $c['documento'] }}</td>
                        <td>{{ $c['cidade'] }}</td>
                        <td class="text-right text-success">R$ {{ number_format($c['total_recebido'], 2, ',', '.') }}</td>
                        <td class="text-right">R$ {{ number_format($c['pendente'], 2, ',', '.') }}</td>
        ...
        @foreach($clientes as $c)
            <div class="mobile-card">
                ...
                <div class="btn-group" style="margin-top:0.5rem;">
                    <a href="{{ route('clientes.show', $c['id']) }}" class="btn btn-outline btn-sm">Perfil</a>
                    <a href="{{ route('clientes.edit', $c['id']) }}" class="btn btn-outline btn-sm">
                   Editar
                </a>
                </div>
            </div>
